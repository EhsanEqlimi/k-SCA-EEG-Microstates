...
a1=FnColNormalizer(randn(D,d1));a2=FnColNormalizer(randn(D,d2));
X1 =a1 * randn(d1,N1); %Generating N1 points in a d1 dim. subspace
X2 = a2 * randn(d2,N2); %Generating N2 points in a d2 dim. subspace
X = [X1 X2];
...
if (Fail == 0)
    ...
    [Grps , SingVals, LapKernel] = FnSpectralClustering(CKSym,n);
...
[Sub,OrthSub]=FnSubspaceCalc(X,Grps(:,1)',1,n);
[Ahat,MinEV]=FnMixingCalc_Ehsan(OrthSub,1,2,2)